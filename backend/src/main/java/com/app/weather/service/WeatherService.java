...
public class WeatherService {
    ...
    public WeatherDTO getCurrentWeather(String city) {
        return fetchWeather(String.format("%s/weather?q=%s&appid=%s&units=metric",
                baseUrl, city, apiKey), city);
    }

    @Cacheable(value = "forecast", key = "#city.toLowerCase()")
    public ForecastDTO getForecast(String city) {
        return fetchForecast(String.format("%s/forecast?q=%s&appid=%s&units=metric",
                baseUrl, city, apiKey), city);
    }
    ...
    public WeatherDTO getCurrentWeatherByCoords(double lat, double lon) {
        return fetchWeather(String.format("%s/weather?lat=%s&lon=%s&appid=%s&units=metric",
                baseUrl, lat, lon, apiKey), "coords");
    }

    public ForecastDTO getForecastByCoords(double lat, double lon) {
        return fetchForecast(String.format("%s/forecast?lat=%s&lon=%s&appid=%s&units=metric",
                baseUrl, lat, lon, apiKey), "coords");
    }
    ...
    private ForecastDTO mapToForecastDTO(Map<String, Object> raw) {
        ...
        List<ForecastDTO.HourlySlot> hourlySlots = list.stream()
            .limit(8).map(this::toHourlySlot).collect(Collectors.toList());

        List<ForecastDTO.DailySummary> dailySummaries = list.stream()
            .filter(s -> s.get("dt_txt").toString().contains("12:00:00"))
            .limit(5).map(s -> toDailySummary(s, list)).collect(Collectors.toList());

        return ForecastDTO.builder()
            .city(   (String) cityMap.get("name"))
            .country((String) cityMap.get("country"))
    ...
    private ForecastDTO.HourlySlot toHourlySlot(Map<String, Object> slot) {
        ...
        return ForecastDTO.HourlySlot.builder()
            .dt(dt).time(TIME_FMT.format(Instant.ofEpochSecond(dt)))
            .temperature(        toDouble(main.get("temp")))
            .feelsLike(          toDouble(main.get("feels_like")))
            .humidity(           toInt(main.get("humidity")))
            .windSpeed(          toDouble(wind.get("speed")) * 3.6)
    ...
    private ForecastDTO.DailySummary toDailySummary(
            Map<String, Object> noon, List<Map<String, Object>> all) {
        ...
        return ForecastDTO.DailySummary.builder()
            .dt(dt).dayName(DAY_FMT.format(Instant.ofEpochSecond(dt)))
            .tempDay(toDouble(main.get("temp"))).tempMin(min).tempMax(max)
            .condition((String) wList.get(0).get("main"))
            .icon(     (String) wList.get(0).get("icon"))
            .humidity( toInt(main.get("humidity")))
